...
#define METHOD_PLAY_AUDIO @"playAudio"
...
- (BOOL)captureRequest:(NSURLRequest *)request webView:(UIWebView *)webView
{
    if ([request.URL.scheme isEqual:@"sharesdk"])
    {
        ...
        else if ([request.URL.host isEqual:@"call"])
        {
            ...
            if (paramsStr)
            {
                if ([methodName isEqualToString:METHOD_PLAY_AUDIO])
                {
                    //播放音效
                    paramsStr = [paramsStr stringByReplacingCharactersInRange:NSMakeRange(0, 1) withString:@""];
...
- (BOOL) playAudio: (NSString *)audioName
{
    NSError *error;
